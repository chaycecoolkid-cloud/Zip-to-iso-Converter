...
def filesystem_allows_debootstrap(target_dir: Path) -> bool:
...
def build_iso_from_zip(zip_path: Path, output_iso: Path) -> None:
    ...
    stage_root = Path("/var/tmp") if Path("/var/tmp").exists() else Path("/tmp")
    if not filesystem_allows_debootstrap(stage_root):
        raise RuntimeError(
            "This environment does not allow a full OS bootstrap. "
            "A writable, exec-capable temp directory is required to build a real bootable ISO."
        )
    ...
    try:
        ...
        if "noexec" in message or "nodev" in message or "Operation not permitted" in message:
            raise RuntimeError(
                ...
                "Use a writable temp directory or a machine that allows debootstrap and chroot."
            ) from exc
        raise
